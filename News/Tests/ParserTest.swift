...
final class ParserTest: XCTestCase {
    func testParse() async throws {
        ...
        XCTAssertEqual("Reuters revealed that Germany and Qatar have hit difficulties in talks over long-term liquefied natural gas (LNG) supply deals amid differences over key conditions, including the duration of any contract. Germany, which aims to cut its carbon emissions by 88% by 2040, is reluctant to commit to Qatar’s conditions to sign deals of at least 20 years to secure the massive LNG volumes it needs to reduce its dependence on Russian gas.\n\nThe post Reuters reveals Germany, Qatar at odds over terms in talks on LNG supply deal appeared first on Reuters News Agency.",
                       result?.description)
    }
    
    func testParseNoContent() async throws {
        let result = try await Fetcher().parse(feed: .theLocalInternational, data: .init(xmlNoContent.utf8)).first
